				...
				if gen > initial_gens:
					...
					if gen >= adapt_gens[-1] + new_delta_window:
						if ((np.around(curr_ratio, decimals=2) == 1
							...
							toolbox.register("mutate", operators.neighbourHyperMutation_all, MUTPB = 1.0, gen_length = relev_links_len, argsortdists=argsortdists, L = L, int_links_indices=int_links_indices, nn_rankings = nn_rankings, hyper_mut=hyper_mut, new_genes=newly_unfixed_indices)
