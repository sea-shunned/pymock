...
for file_path in data_files:
	...
	for index_d, delta in enumerate(delta_vals):
		...
		for func in funcs:
			...
			if save_results:
				# Save array data
				np.savetxt(filename+"-fitness-sr"+str(sr_vals[index_d])+"-interval.csv", fitness_array, delimiter=",")
				np.savetxt(filename+"-hv-sr"+str(sr_vals[index_d])+"-interval.csv", hv_array, delimiter=",")
				np.savetxt(filename+"-ari-sr"+str(sr_vals[index_d])+"-interval.csv", ari_array, delimiter=",")
				np.savetxt(filename+"-numclusts-sr"+str(sr_vals[index_d])+"-interval.csv", numclusts_array, delimiter=",")
				np.savetxt(filename+"-time-sr"+str(sr_vals[index_d])+"-interval.csv", time_array, delimiter=",")

				# Pickle delta triggers
				# No triggers for normal delta-MOCK
				if strat_name != "main_base":
					with open(filename+"-triggers-sr"+str(sr_vals[index_d])+"-interval.csv","w") as f:
